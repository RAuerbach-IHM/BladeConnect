...
{
    public class WheatNetTCPClient : IDisposable
    {
        public WheatNetTCPClient()
        {
            Messenger.Default.Register<NetworkClientSendMessage>(this, OnMessageReceived_SendMessage);

            parseTCPDataReceived = new ParseTCPDataReceived();
        }
        ...
        ParseTCPDataReceived parseTCPDataReceived;
        ...
        public void StartClient(WNIPDevice wnipDevice)
        {
            ...
            switch (wnipDevice.DeviceType)
            {
                case DEVICE_TYPE.WHEATNET_BLADE:
                    ...
                    break;
                case DEVICE_TYPE.WHEATNET_SWITCHBLADE:
                    Console.WriteLine("WheatNetTCPClient_UDP HIT");
                    _wnipDevice = wnipDevice;
                    //RemoteEvents = remoteEvents;
                    CreateClient_UDP(_wnipDevice.IP, _wnipDevice.Port);
                    break;
                default:
        ...
        private void CreateClient(string server, int port)
        {
            ...
            catch (ArgumentNullException e)
            {
                ...
                MessageBox.Show("Connection Error (Null)" + e.ToString());
            }
            catch (SocketException e)
            {
                Console.WriteLine("SocketException: {0}", e);
                MessageBox.Show("Connection Error (Socket) " + e.ToString());
            }
            ...
            var t = Task.Factory.StartNew(async () =>
            {
                ...
                try
                {
                    while (Stream != null)
                    {
                        ...
                        parseTCPDataReceived.Parse(_wnipDevice.ID, responseData);
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show("TcpClient Closed");
                }
            });
